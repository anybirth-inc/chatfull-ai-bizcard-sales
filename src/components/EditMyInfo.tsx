...
export function EditMyInfo() {
  ...
  const { myCompanyInfo, setMyCompanyInfo, isDoubleSided, currentSide, setCurrentSide } = useStore((state) => ({
    myCompanyInfo: state.myCompanyInfo,
    setMyCompanyInfo: state.setMyCompanyInfo,
  ...
  const [formData, setFormData] = useState({
    ...
    companyNameKana: myCompanyInfo?.companyNameKana || '',
    firstName: myCompanyInfo?.firstName || '',
    lastName: myCompanyInfo?.lastName || '',
    firstNameKana: myCompanyInfo?.firstNameKana || '',
    lastNameKana: myCompanyInfo?.lastNameKana || '',
    personalPhone: myCompanyInfo?.personalPhone || '',
    companyPhone: myCompanyInfo?.companyPhone || '',
    faxNumber: myCompanyInfo?.faxNumber || '',
    email: myCompanyInfo?.email || '',
    address: myCompanyInfo?.address || '',
    position: myCompanyInfo?.position || '',
    website: myCompanyInfo?.website || '',
    services: myCompanyInfo?.services || [],
  });
    ...
    <div className="max-w-2xl mx-auto">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <div className="grid grid-cols-1 gap-6">
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700">
              会社名（ふりがな） <span className="text-red-500">*</span>
            </label>
            <input
              type="text"
